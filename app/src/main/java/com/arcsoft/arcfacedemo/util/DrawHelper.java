        ...
        switch (cameraDisplayOrientation) {
            ...
            case 90:
                ...
                if (cameraId == Camera.CameraInfo.CAMERA_FACING_FRONT) {
                    ...
                    newRect.top = previewWidth - rect.left;
                    newRect.bottom = rect.top + width;
                }
                break;
            case 180:
            ...
            case 270:
                ...
                if (cameraId == Camera.CameraInfo.CAMERA_FACING_FRONT) {
                    ...
                    newRect.bottom = canvasHeight - rect.left;
//                    int width = rect.width();
//                    newRect.top = canvasHeight - newRect.bottom;
//                    newRect.bottom = newRect.top + width;
                }
                break;
            default:
                break;
     ...
     * @param height
     */
    public static synchronized void drawFaceRect(Canvas canvas, DrawInfo drawInfo, int color, int faceRectThickness, int height) {
        ...
        mPath.moveTo(rect.left, rect.top + (rect.height()>>2));
        mPath.lineTo(rect.left, rect.top);
        mPath.lineTo(rect.left + (rect.width()>>2), rect.top);
        //右上
        mPath.moveTo(rect.right - (rect.width()>>2), rect.top);
        mPath.lineTo(rect.right, rect.top);
        mPath.lineTo(rect.right, rect.top + (rect.height()>>2));
        //右下
        mPath.moveTo(rect.right, rect.bottom - (rect.height()>>2));
        mPath.lineTo(rect.right, rect.bottom);
        mPath.lineTo(rect.right - (rect.width()>>2), rect.bottom);
        //左下
        mPath.moveTo(rect.left + (rect.width()>>2), rect.bottom);
        mPath.lineTo(rect.left, rect.bottom);
        mPath.lineTo(rect.left, rect.bottom - (rect.height()>>2));
        canvas.drawPath(mPath, paint);


        if (drawInfo.getName() == null) {
            paint.setStyle(Paint.Style.FILL_AND_STROKE);
            paint.setTextSize((rect.width()>>3));
